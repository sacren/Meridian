/// <reference types="cypress" />

/**
 * End-to-end proof that the campaign dashboard's "Manage" section exposes a working
 * link into segments: a freshly-registered owner creates a campaign, then follows
 * the dashboard's "Segments" card to the campaign-scoped segments page.
 *
 * Each run registers a brand-new user with a unique email, so it needs no seeded state.
 */
describe('campaign dashboard segments link', () => {
    const password = 'password1234';

    function register(name: string): void {
        const email = `e2e-${Date.now()}-${Math.random().toString(36).slice(2, 8)}@example.test`;

        cy.visit('/register');
        cy.get('input[name="name"]').type(name);
        cy.get('input[name="email"]').type(email);
        cy.get('input[name="password"]').type(password);
        cy.get('input[name="password_confirmation"]').type(password);
        cy.get('[data-test="register-user-button"]').click();

        cy.url().should('not.include', '/register');
    }

    it('navigates from the dashboard to the campaign segments page', () => {
        const campaignName = `Acme Outreach ${Date.now()}`;

        register('E2E Owner');

        cy.visit('/campaigns');
        cy.get('[data-test="campaign-name-input"]').type(campaignName);
        cy.get('[data-test="create-campaign-button"]').click();

        cy.get('[data-test="campaign-dashboard"]').should('exist');

        cy.location('pathname').then((campaignPath) => {
            cy.get('[data-test="segments-link"]').should('be.visible').click();

            // We land on the campaign-scoped segments page, with its builder form.
            cy.location('pathname').should('eq', `${campaignPath}/segments`);
            cy.get('[data-test="segment-form"]').should('exist');
            cy.get('[data-test="segment-name-input"]').should('exist');
        });
    });
});
